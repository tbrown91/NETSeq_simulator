 void sim_polIITwoWindows(const double init,const double elong,const double pause1,const double restart1,const double pause2,const double restart2,const double term_prob,const double window_size,vector<int>& NETseqDist,vector<int>& PROseqDist,const int G,const int W,const int num_cells,double dt,const int num_timeSteps,int& num_init,int& num_term,int& num_finished){
   ...
   for (int cell=0;cell<num_cells;++cell){
     for (int timeStep=1;timeStep<num_timeSteps;++timeStep){
       ...
       move_polIITwoWindows(polII_location[cell],polII_status[cell],elong*dt,pause1*dt,restart1*dt,pause2*dt,restart2*dt,window_size,term_prob,num_term);
