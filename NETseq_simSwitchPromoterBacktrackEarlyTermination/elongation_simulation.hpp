 void sim_polII(const double act,const double inact,const double init,const double elong,const double pause,const double restart,const double backtrack,const double backtrack_restart,const double termination, vector<int>& movingStalledBacktrackedPolIIDist, vector<int>& movingStalledPolIIDist, vector<int>& movingPolIIDist,const int G,const int W,const int num_cells,double dt,const int num_timeSteps,int& num_init,int& num_term,int& num_finished){
   ...
   for (int cell=0;cell<num_cells;++cell){
     for (int timeStep=1;timeStep<num_timeSteps;++timeStep){
       ...
       check_bind(polII_location[cell],polII_status[cell],promoter[cell],act*dt,inact*dt,init*dt,elong*dt,W,num_init,num_term);
     }
