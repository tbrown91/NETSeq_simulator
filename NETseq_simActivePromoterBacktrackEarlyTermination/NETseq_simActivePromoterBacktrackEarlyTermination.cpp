...
int main(int argc,char* argv[]){
  ...
  out_file<<"INIT\tELONG\tPAUSE\tRESTART\tBACKTRACK\tBACKTRACK_RESTART\tTERMINATION_WINDOW\tNUM_INIT\tNUM_TERM\tNUM_FINISHED"<<endl;
  ...
  choose_intParam(sample_params[6],0,G,num_params);//Mean early termination window
  ...
  for (int it=0;it<num_params;++it){
    ...
    sim_polII(sample_params[0][it],sample_params[1][it],sample_params[2][it],sample_params[3][it],sample_params[4][it],sample_params[5][it],sample_params[6][it],movingStalledBacktrackedPolIIDist,movingStalledPolIIDist,movingPolIIDist,G,W,num_cells,dt,num_timeSteps,num_init,num_term,num_finished);
