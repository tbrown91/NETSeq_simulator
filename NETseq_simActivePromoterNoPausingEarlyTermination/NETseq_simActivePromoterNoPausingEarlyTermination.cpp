...
int main(int argc,char* argv[]){
  ...
  out_file<<"INIT\tELONG\tTERMINATION_SIZE\tNUM_INIT\tNUM_TERM\tNUM_FINISHED"<<endl;
  ...
  choose_intParam(sample_params[2],0,G,num_params);//Elongation rate (b/min)
  ...
  for (int it=0;it<num_params;++it){
    ...
    sim_polII(sample_params[0][it],sample_params[1][it],sample_params[2][it],NETseqDist,PROseqDist,G,W,num_cells,dt,num_timeSteps,num_init,num_term,num_finished);
