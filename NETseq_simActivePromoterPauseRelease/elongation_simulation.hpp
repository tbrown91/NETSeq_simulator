 void sim_polII(const double init,const double elong,const int pause_location,const double pause_restart, vector<int>& NETseqDist,vector<int>& PROseqDist,const int G,const int W,const int num_cells,double dt,const int num_timeSteps,int& num_init,int& num_term,int& num_finished){
   ...
   for (int cell=0;cell<num_cells;++cell){
     if (gsl_rng_uniform(r)<init*dt){
       ...
       if (polII_location[cell].back()>=pause_location){
         polII_location[cell].back()=pause_location;
         polII_status[cell].back()=1;
       }
     }
   }
   ...
   for (int cell=0;cell<num_cells;++cell){
     for (int timeStep=1;timeStep<num_timeSteps;++timeStep){
       ...
       move_polII(polII_location[cell],polII_status[cell],elong*dt,pause_location,pause_restart*dt);
       check_overlap(polII_location[cell],polII_status[cell],G,W,num_term,num_finished);
       check_bind(polII_location[cell],polII_status[cell],promoter[cell],init*dt,elong*dt,pause_location,W,num_init,num_term);
     }
