void move_polIITwoWindows(vector<int>& polII_location,vector<int>& polII_status,const double elong,const double pause1,const double restart1,const double backtrack1,const double backtrack_restart1,const double pause2,const double restart2,const double backtrack2,const double backtrack_restart2,const int window_size,const double term,int& num_term){
    ...
    for (int it=0;it<(int)polII_location.size();++it){
      if (gsl_rng_uniform(r)<term){//Remove PolII
        ...
        if (polII_status[it]==0){//PolII is moving
          ...
          if (polII_location[it]<(int)window_size){
            if (gsl_rng_uniform(r)<backtrack_restart1){
              polII_status[it]=0;
              polII_location[it]+=gsl_ran_poisson(r,elong);
            }
          }else{
            if (gsl_rng_uniform(r)<backtrack_restart2){
              polII_status[it]=0;
              polII_location[it]+=gsl_ran_poisson(r,elong);
            }
          }
    ...
    for (int k=(int)polII_location.size()-1;k>-1;--k){
      ...
      if (k>0){
        if (polII_location[k-1]+W>polII_location[k]){//Overlapping polII
          ...
          polII_status[k-1]=1;
          if (polII_status[k]>0){//remove PolII
            ++num_term;
            polII_location.erase(polII_location.begin()+k);
            polII_status.erase(polII_status.begin()+k);
          }
