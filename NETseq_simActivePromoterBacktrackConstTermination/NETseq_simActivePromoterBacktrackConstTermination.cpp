...
int main(int argc,char* argv[]){
  ...
  out_file<<"INIT\tELONG\tPAUSE\tRESTART\tBACKTRACK\tBACKTRACK_RESTART\tTERM\tNUM_INIT\tNUM_TERM\tNUM_FINISHED"<<endl;
  ...
  choose_param(sample_params[2],0.01,60.0,num_params);//Pause rate (/min)
  choose_param(sample_params[3],0.01,60.0,num_params);//Restart rate (/min)
  choose_param(sample_params[4],0.01,60.0,num_params);//Backtrack rate (/min)
  choose_param(sample_params[5],0.01,60.0,num_params);//Backtrack restart rate (/min)
  choose_param(sample_params[6],0.001,1.0,num_params);//Early termination probability per min
  ...
  for (int it=0;it<num_params;++it){
    ...
    sim_polII(sample_params[0][it],sample_params[1][it],sample_params[2][it],sample_params[3][it],sample_params[4][it],sample_params[5][it],sample_params[6][it],movingStalledBacktrackedDist,movingStalledDist,movingDist,G,W,num_cells,dt,num_timeSteps,num_init,num_term,num_finished);
