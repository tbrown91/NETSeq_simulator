void move_polII(vector<int>& polII_location,vector<int>& polII_status,const double elong,const double pause,const double restart,const double backtrack,const double backtrack_restart,const double term,int& num_term){
  if (polII_location.size()>0){
    for (int k=0;k<(int)polII_location.size();++k){
      if (gsl_rng_uniform(r)<term){//Remove PolII
        ...
        }else if(polII_status[k]==1){//PolII is paused
          if (gsl_rng_uniform(r)<restart){//Restart polII
            polII_status[k]=0;
            polII_location[k]+=gsl_ran_poisson(r,elong);
          }else if(gsl_rng_uniform(r)<backtrack){//PolII enters backtrack
            polII_status[2];
          }
