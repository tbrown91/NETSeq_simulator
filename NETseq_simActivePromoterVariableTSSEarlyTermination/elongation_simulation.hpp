 void sim_polII(const double init,const double elong,const double pause,const double restart, const int TSS_mu,const double TSS_sigma,const double termination,vector<int>& NETseqDist,vector<int>& PROseqDist,const int G,const int W,const int num_cells,double dt,const int num_timeSteps,int& num_init,int& num_term,int& num_finished){
   ...
   for (int cell=0;cell<num_cells;++cell){
     if (gsl_rng_uniform(r)<init*dt){
       ...
       polII_location[cell].push_back(TSS_start+gsl_ran_poisson(r,elong*dt));
