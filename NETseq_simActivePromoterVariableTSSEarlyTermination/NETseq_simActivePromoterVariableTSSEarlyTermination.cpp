...
int main(int argc,char* argv[]){
  ...
  out_file<<"INIT\tELONG\tPAUSE\tRESTART\tTSS_mu\tTSS_sigma\tTERMINATION_SIZE\tNUM_INIT\tNUM_TERM\tNUM_FINISHED"<<endl;
  ...
  choose_intParam(sample_params[4],-250,250,num_params);//Centre of PolII binding (relative to annotated TSS)
  choose_param(sample_params[5],0.01,250.0,num_params);//Standard deviation of PolII binding
  choose_intParam(sample_params[6],0,G,num_params);//Mean location of early termination
  ...
  for (int it=0;it<num_params;++it){
    ...
    sim_polII(sample_params[0][it],sample_params[1][it],sample_params[2][it],sample_params[3][it],sample_params[4][it],sample_params[5][it],sample_params[6][it],NETseqDist,PROseqDist,G,W,num_cells,dt,num_timeSteps,num_init,num_term,num_finished);
