...
int main(int argc,char* argv[]){
  ...
  out_file<<"INIT\tELONG\tPAUSE1\tRESTART1\tPAUSE2\tRESTART2\tTSS_mu\tTSS_sigma\tWINDOW_SIZE\tTERMINATION_SIZE\tNUM_INIT\tNUM_TERM\tNUM_FINISHED"<<endl;
  ...
  choose_param(sample_params[4],0.01,60.0,num_params);//Second pause rate (/min)
  choose_param(sample_params[5],0.01,60.0,num_params);//Second restart rate (/min)
  choose_intParam(sample_params[6],-250,250,num_params);//Start site target, relative to annotated TSS
  choose_param(sample_params[7],0.01,2500.0,num_params);//Standard deviation of start site selection
  choose_intParam(sample_params[8],0,G,num_params);//Window size (bp)
  choose_intParam(sample_params[9],0,G,num_params);//Termination window (bp)
  ...
  for (int it=0;it<num_params;++it){
    ...
    sim_polIITwoWindows(sample_params[0][it],sample_params[1][it],sample_params[2][it],sample_params[3][it],sample_params[4][it],sample_params[5][it],sample_params[6][it],sample_params[7][it],sample_params[8][it],sample_params[9][it],NETseqDist,PROseqDist,G,W,num_cells,dt,num_timeSteps,num_init,num_term,num_finished);
