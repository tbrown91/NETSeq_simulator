 void sim_polIITwoWindows(const double act,const double inact,const double init,const double elong,const double pause1,const double restart1,const double pause2,const double restart2,const double TSS_mu,const double TSS_sigma,const double window_size,const double termination,vector<int>& NETseqDist,vector<int>& PROseqDist,const int G,const int W,const int num_cells,double dt,const int num_timeSteps,int& num_init,int& num_term,int& num_finished){
   ...
   for (int cell=0;cell<num_cells;++cell){
     for (int timeStep=1;timeStep<num_timeSteps;++timeStep){
       ...
       check_bindTwoWindows(polII_location[cell],polII_status[cell],promoter[cell],act*dt,inact*dt,init*dt,elong*dt,TSS_mu,TSS_sigma,W,num_init,num_term,G);
     }

     for (int it=0;it<(int)polII_location[cell].size();++it){
       if ((polII_location[cell][it]>-251)&&(polII_location[cell][it])){
