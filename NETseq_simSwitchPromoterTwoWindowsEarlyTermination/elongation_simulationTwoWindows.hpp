 void sim_polIITwoWindows(const double act,const double inact,const double init,const double elong,const double pause1,const double restart1,const double pause2,const double restart2,const double window_size,const double termination,vector<int>& NETseqDist, vector<int>& PROseqDist,const int G,const int W,const int num_cells,double dt,const int num_timeSteps,int& num_init,int& num_term,int& num_finished){
  ...
  for (int cell=0;cell<num_cells;++cell){
    for (int timeStep=1;timeStep<num_timeSteps;++timeStep){
       ...
       check_bindTwoWindows(polII_location[cell],polII_status[cell],promoter[cell],act*dt,inact*dt,init*dt,elong*dt,W,num_init,num_term);
    }
